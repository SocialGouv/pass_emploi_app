                ...
                builder: (dateDerniereConsultation) {
                  return BaseCard(
                    ...
                    onTap: () => switch (viewModel.type) {
                      OffreType.emploi => Navigator.of(context).push(OffreEmploiDetailsPage.materialPageRoute(
                          viewModel.id,
                          fromAlternance: false,
                        )),
                      OffreType.alternance => Navigator.of(context).push(OffreEmploiDetailsPage.materialPageRoute(
                          viewModel.id,
                          fromAlternance: true,
                        )),
                      OffreType.immersion =>
                        Navigator.of(context).push(ImmersionDetailsPage.materialPageRoute(viewModel.id)),
                      OffreType.serviceCivique =>
                        Navigator.of(context).push(ServiceCiviqueDetailPage.materialPageRoute(viewModel.id)),
                    },
                    complements: [
                      if (viewModel.localisation != null) CardComplement.place(text: viewModel.localisation!),
