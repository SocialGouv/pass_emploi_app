...
void main() {
  group("create when update action...", () {
    ...
    test(
        "needs no modification should dismiss bottom sheet without updating action",
        () {
      // Given
      final store = givenState()
          .withAction(mockUserAction(id: 'actionId'))
          .updateActionNoNeedToUpdate()
          .store();
  ...
  group("create when delete action ...", () {
    test("succeeds should dismiss bottom sheet", () {
      ...
      // Then
      expect(viewModel.deleteDisplayState,
    ...
    test("fails should display error", () {
      ...
      // Then
      expect(
          viewModel.deleteDisplayState, DeleteDisplayState.SHOW_DELETE_ERROR);
    });
  ...
  test("create when delete action fails should display error", () {
    ...
    final store = Store<AppState>(
      ...
      initialState: AppState.initialState().copyWith(
        userActionListState:
            UserActionListSuccessState([mockUserAction(id: 'id')]),
    ...
    // When
    final viewModel =
        UserActionDetailsViewModel.createFromUserActionListState(store, 'id');

    // Then
    expect(viewModel.displayState,
        UserActionDetailsDisplayState.SHOW_DELETE_ERROR);
  });

  test(
      'refreshStatus when update status has changed should dispatch a UpdateActionStatus',
      () {
    // Given
    final store = StoreSpy.withState(givenState()
        .loggedInMiloUser()
        .withAction(mockUserAction(id: 'actionId')));
  ...
  test(
      'refreshStatus when update status has not changed should dispatch UserActionNoUpdateNeededAction',
      () {
    // Given
    final store = StoreSpy.withState(givenState()
        .loggedInMiloUser()
  ...
  test('onDelete should dispatch UserActionDeleteRequestAction', () {
    ...
    final store = StoreSpy.withState(
        givenState().withAction(mockUserAction(id: 'actionId')));
    final viewModel = UserActionDetailsViewModel.createFromUserActionListState(
        store, 'actionId');
    ...
    expect((store.dispatchedAction as UserActionDeleteRequestAction).actionId,
        'actionId');
  });

  test('deleteFromList should dispatch UserActionDeleteFromListAction',
      () async {
    // Given
    final store = StoreSpy.withState(
        givenState().withAction(mockUserAction(id: 'actionId')));
